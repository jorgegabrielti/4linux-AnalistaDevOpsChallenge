...
async function createChallenges () {
  ...
  await Promise.all(
    challenges.map(async ({ name, category, description, difficulty, hint, hintUrl, mitigationUrl, key, disabledEnv, tutorial, tags }: Challenge) => {
      ...
      description = description.replace('Coffee-sh.op', config.get('application.domain'))
      ...
      hint = hint.replace(/OWASP Coffee Shop's/, `${config.get('application.name')}'s`)
...
async function createProducts () {
  ...
  function customizeRetrieveBlueprintChallenge (hint: string, customProduct: Product) {
    return hint.replace(/OWASP Coffee Shop Logo \(3D-printed\)/g, customProduct.name)
  }
}
